...
# EOC
two_way_anova_doc <- aov(doc ~ treatment * sample_date, data = mcse_data)
summary(two_way_anova_doc)

doc_season_effects <- lm(doc~sample_date + 0, data = mcse_data)
summary(doc_season_effects)
confint(doc_season_effects)

# Post-Hoc Test
tukey_doc <- TukeyHSD(two_way_anova_doc, "treatment", ordered = FALSE, conf.level = 0.95)
tukey_doc

# EON
two_way_anova_don <- aov(don ~ treatment * sample_date, data = mcse_data)
summary(two_way_anova_don)
...
# EOC
two_way_anova_doc <- aov(doc ~ system * sample_date, data = mcse_data)
summary(two_way_anova_doc)

# Post-hoc test (treatments)
tukey_doc_category <- TukeyHSD(two_way_anova_doc, "system", ordered = FALSE, conf.level = 0.95)
tukey_doc_category

# EON
two_way_anova_don <- aov(don ~ system * sample_date, data = mcse_data)
summary(two_way_anova_don)
...
mcse_data$doc <- as.numeric(mcse_data$doc)
mcse_data$don <- as.numeric(mcse_data$don)
...
mcse_data_nona <- mcse_data %>% drop_na(doc)

pcor_fixation_diversity_mcse <- pcor.test(mcse_data_nona$shan, mcse_data_nona$n.fix, mcse_data_nona[,c("doc", "mbc", "mbn")], method = "spearman")
...
merge_diversity_effects <- merge_diversity_effects %>% reduce(full_join)
...
merge_tukey_diversity_mcse <- merge_tukey_diversity_mcse  %>% reduce(full_join)
...
merge_flnf_effects <- merge_flnf_effects %>% reduce(full_join)
...
merge_tukey_flnf_mcse <- merge_tukey_flnf_mcse  %>% reduce(full_join)
...
merge_mbc_effects <- merge_mbc_effects %>% reduce(full_join)
...
merge_tukey_mbc_mcse <- list(letters_df_mbc_summer,
                              letters_df_mbc_fall)
merge_tukey_mbc_mcse <- merge_tukey_mbc_mcse  %>% reduce(full_join)
...
merge_mbn_effects <- merge_mbn_effects %>% reduce(full_join)
...
merge_tukey_mbn_mcse <- list(letters_df_mbn_summer,
                             letters_df_mbn_fall)
merge_tukey_mbn_mcse <- merge_tukey_mbn_mcse  %>% reduce(full_join)
...
mcse_mbn_plot <- mcse_data %>%
  ...
  scale_fill_manual(values=c('#EEAC4E','#D7785D','#9C7C50', '#577753',
                    ...
                    labels = c("T1: Conventional Corn/Soy/Wheat", 
                               ...
                               "SF: Successional Forest", "DF: Deciduous Forest", 
                               "CF: Coniferous Forest")) +
  ylab("Microbial Biomass Nitrogen (\u00b5g/g soil)") +
  xlab("") +
  theme_bw() +
  theme(axis.title.x = element_text(size = 14), 
        title = element_text(size = 14),
...
## EOC ####
# Obtain mean doc values for each system (annual, perennial, forest)
lm_doc_category_summer <- lm(doc ~ system + 0, data = mcse_data_summer)
summary(lm_doc_category_summer)

lm_doc_category_fall <- lm(doc ~ system + 0, data = mcse_data_fall)
summary(lm_doc_category_fall)

# Add model estimates into dataframe for GGPLOT
est <- c(152.023, 158.195, 175.488)
system <- c("annual", "perennial", "forest")
sample_date <- c("summer", "summer", "summer")
category_doc_effects_summer <- data.frame(est, system, sample_date)

est <- c(47.104, 52.363, 50.610)
system <- c("annual", "perennial", "forest")
sample_date <- c("fall", "fall", "fall")
category_doc_effects_fall <- data.frame(est, system, sample_date)

merge_doc_effects <- list(category_doc_effects_summer,
                          category_doc_effects_fall)
merge_doc_effects <- merge_doc_effects %>% reduce(full_join)


# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: Summer Samples 
letters_df_doc_summer <- data.frame(multcompLetters(TukeyHSD(aov(doc ~ treatment, data = mcse_data_summer))$treatment[,4])$Letters)

colnames(letters_df_doc_summer)[1] <- "Letter" #Reassign column name
letters_df_doc_summer$treatment <- rownames(letters_df_doc_summer) 
letters_df_doc_summer$placement <- c(220, 220, 220, 220, 220, 220, 220, 220)
value_map <- c("T1" = "annual",
               "T3" = "annual",
               "T4" = "annual",
               "T5" = "perennial",
...
value_map_summer <- c("T1" = "summer",
                      ...
                      "SF" = "summer",
                      "CF" = "summer")
letters_df_doc_summer$system <- value_map[letters_df_doc_summer$treatment]
letters_df_doc_summer$system <- factor(letters_df_doc_summer$system, levels = c("annual", "perennial", "forest"))
letters_df_doc_summer$sample_date <- value_map_summer[letters_df_doc_summer$treatment]

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: Fall Samples 
letters_df_doc_fall <- data.frame(multcompLetters(TukeyHSD(aov(doc ~ treatment, data = mcse_data_fall))$treatment[,4])$Letters)

colnames(letters_df_doc_fall)[1] <- "Letter" #Reassign column name
letters_df_doc_fall$treatment <- rownames(letters_df_doc_fall) 
letters_df_doc_fall$placement <- c(90, 90, 90, 90, 90, 90, 90, 90)
value_map <- c("T1" = "annual",
               "T3" = "annual",
               "T4" = "annual",
               "T5" = "perennial",
...
value_map_fall <- c("T1" = "fall",
                    ...
                    "SF" = "fall",
                    "CF" = "fall")
letters_df_doc_fall$system <- value_map[letters_df_doc_fall$treatment]
letters_df_doc_fall$system <- factor(letters_df_doc_fall$system, levels = c("annual", "perennial", "forest"))
letters_df_doc_fall$sample_date <- value_map_fall[letters_df_doc_fall$treatment]

# Merge all letters together
merge_tukey_doc_mcse <- list(letters_df_doc_summer,
                             letters_df_doc_fall)
merge_tukey_doc_mcse <- merge_tukey_doc_mcse  %>% reduce(full_join)

# Reorder Treatments
merge_doc_effects$system <- factor(merge_doc_effects$system, levels = c("annual", "perennial", "forest"))
merge_tukey_doc_mcse$system <- factor(merge_tukey_doc_mcse$system, levels = c("annual", "perennial", "forest"))

merge_doc_effects$sample_date <- factor(merge_doc_effects$sample_date, levels = c("summer", "fall"))
merge_tukey_doc_mcse$sample_date <- factor(merge_tukey_doc_mcse$sample_date, levels = c("summer", "fall"))


# Plot doc across all treatments-- with Post-hoc comparisons across treatment and system-model mean estimates
mcse_doc_plot <- mcse_data %>%
  ggplot(aes(x=treatment, y = doc, fill = treatment)) +
  geom_boxplot(alpha = 0.90, outlier.shape = NA) +
  geom_text(data = merge_tukey_doc_mcse, 
            aes(x=treatment, y=placement, label=Letter), 
            size =5.5, color="black", fontface="bold") +
  facet_grid(sample_date ~ system, scales = 'free_x') +
  geom_hline(aes(yintercept=est), linetype = 'dashed', 
             col = 'black', linewidth = 1.25,
             data=merge_doc_effects) +
  scale_fill_manual(values=c('#EEAC4E','#D7785D','#9C7C50', '#577753',
                             '#1A4C76','#86A592','#577B96','#B0381D'), 
                    name = "MCSE Treatment",
                    labels = c("T1: Conventional Corn/Soy/Wheat", 
                               ...
                               "SF: Successional Forest", "DF: Deciduous Forest", 
                               "CF: Coniferous Forest")) +
  ylab("Extractable Organic Carbon (\u00b5g/g soil)") +
  xlab("") +
  theme_bw() +
  theme(axis.title.x = element_text(size = 14), 
        title = element_text(size = 14),
...
lm_don_category_summer <- lm(don ~ system + 0, data = mcse_data_summer)
summary(lm_don_category_summer)

# Add model estimates into dataframe for GGPLOT
est <- c(15.389, 9.586, 8.019)
system <- c("annual", "perennial", "forest")
category_don_effects_summer <- data.frame(est, system)

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: Summer Samples 
letters_df_don_summer <- data.frame(multcompLetters(TukeyHSD(aov(don ~ treatment, data = mcse_data_summer))$treatment[,4])$Letters)

colnames(letters_df_don_summer)[1] <- "Letter" #Reassign column name
letters_df_don_summer$treatment <- rownames(letters_df_don_summer) 
letters_df_don_summer$placement <- c(30, 30, 30, 30, 30, 30, 30, 30)
value_map <- c("T1" = "annual",
               "T3" = "annual",
               "T4" = "annual",
               "T5" = "perennial",
               ...
               "SF" = "forest",
               "CF" = "forest")
letters_df_don_summer$system <- value_map[letters_df_don_summer$treatment]
letters_df_don_summer$system <- factor(letters_df_don_summer$system, levels = c("annual", "perennial", "forest"))

# Reorder Treatments
mcse_data_summer$treatment <- factor(mcse_data_summer$treatment, levels = c("T1", "T3", "T4", "T5", "T7", "SF", "DF", "CF"))
mcse_data_summer$system <- factor(mcse_data_summer$system, levels = c("annual", "perennial", "forest"))
category_don_effects_summer$system <- factor(category_don_effects_summer$system, levels = c("annual", "perennial", "forest"))

# Plot don across all treatments in the Summer -- with Post-hoc comparisons across treatment and system-model mean estimates
mcse_don_plot <- mcse_data_summer %>%
  ggplot(aes(x=treatment, y = don, fill = treatment)) +
  geom_boxplot(alpha = 0.90, outlier.shape = NA) +
  geom_text(data = letters_df_don_summer, 
            aes(x=treatment, y=placement, label=Letter), 
            size =5.5, color="black", fontface="bold") +
  facet_grid(~system, scales = 'free_x') +
  geom_hline(aes(yintercept=est), linetype = 'dashed', 
             col = 'black', linewidth = 1.25,
             data=category_don_effects_summer) +
  scale_fill_manual(values=c('#EEAC4E','#D7785D','#9C7C50', '#577753',
                             '#1A4C76','#86A592','#577B96','#B0381D'), 
                    name = "MCSE Treatment",
                    labels = c("T1: Conventional Corn/Soy/Wheat", 
                               ...
                               "SF: Successional Forest", "DF: Deciduous Forest", 
                               "CF: Coniferous Forest")) +
  ylab("Extractable Total Nitrogen (\u00b5g/g soil)") +
  xlab("") +
...
en_coord_cont_sig$name <- c("Soil.Moisture", "N.fix", "EOC", "ETN")
...
df_ell.treatment$treatment <- factor(df_ell.treatment$treatment, levels = c("T1", "T3", "T4", "T5", "T7", "SF", "CF", "DF"))
...
# Post-Hoc testing: Both treatment and incubation
tukey_mbc_microcosm_treatment <- TukeyHSD(two_way_anova_mbc_microcosm, "treatment", ordered = FALSE, conf.level = 0.95)
tukey_mbc_microcosm_treatment

tukey_mbc_microcosm_incubation <- TukeyHSD(two_way_anova_mbc_microcosm, "incubation", ordered = FALSE, conf.level = 0.95)
tukey_mbc_microcosm_incubation
...
# EOC
two_way_anova_doc_microcosm <- aov(doc ~ treatment * incubation, data = microcosm_data)
summary(two_way_anova_doc_microcosm)

# Post-Hoc testing: Both treatment and incubation
tukey_doc_microcosm_treatment <- TukeyHSD(two_way_anova_doc_microcosm, "treatment", ordered = FALSE, conf.level = 0.95)
tukey_doc_microcosm_treatment

tukey_doc_microcosm_incubation <- TukeyHSD(two_way_anova_doc_microcosm, "incubation", ordered = FALSE, conf.level = 0.95)
tukey_doc_microcosm_incubation

# ETN
two_way_anova_don_microcosm <- aov(don ~ treatment * incubation, data = microcosm_data)
summary(two_way_anova_don_microcosm)

# Post-Hoc testing: Both treatment by incubation (due to significant interaction)
one_way_anova_don_microcosm_control <- aov(don ~ treatment, data = microcosm_data_control)
tukey_don_microcosm_control_treatment <- TukeyHSD(one_way_anova_don_microcosm_control, "treatment", ordered = FALSE, conf.level = 0.95)
tukey_don_microcosm_control_treatment

one_way_anova_don_microcosm_moderate <- aov(don ~ treatment, data = microcosm_data_moderate)
tukey_don_microcosm_moderate_treatment <- TukeyHSD(one_way_anova_don_microcosm_moderate, "treatment", ordered = FALSE, conf.level = 0.95)
tukey_don_microcosm_moderate_treatment

one_way_anova_don_microcosm_high <- aov(don ~ treatment, data = microcosm_data_high)
tukey_don_microcosm_high_treatment <- TukeyHSD(one_way_anova_don_microcosm_high, "treatment", ordered = FALSE, conf.level = 0.95)
tukey_don_microcosm_high_treatment
...
tukey_mbc_microcosm_category <- TukeyHSD(two_way_anova_mbc_microcosm_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_mbc_microcosm_category
...
# Post-Hoc testing: Both treatment by incubation (due to significant interaction)
one_way_anova_mbn_microcosm_control_category <- aov(mbn ~ system, data = microcosm_data_control)
tukey_mbn_microcosm_control_category <- TukeyHSD(one_way_anova_mbn_microcosm_control_category, "system", ordered = FALSE, conf.level = 0.95)
...
# EOC
two_way_anova_doc_microcosm_category <- aov(doc ~ system * incubation, data = microcosm_data)
summary(two_way_anova_doc_microcosm_category)

# Post-Hoc testing: Both treatment by incubation (due to significant interaction)
one_way_anova_doc_microcosm_control_category <- aov(doc ~ system, data = microcosm_data_control)
tukey_doc_microcosm_control_category <- TukeyHSD(one_way_anova_doc_microcosm_control_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_doc_microcosm_control_category

one_way_anova_doc_microcosm_moderate_category <- aov(doc ~ system, data = microcosm_data_moderate)
tukey_doc_microcosm_moderate_category <- TukeyHSD(one_way_anova_doc_microcosm_moderate_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_doc_microcosm_moderate_category

one_way_anova_doc_microcosm_high_category <- aov(doc ~ system, data = microcosm_data_high)
tukey_doc_microcosm_high_category <- TukeyHSD(one_way_anova_doc_microcosm_high_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_doc_microcosm_high_category

# ETN
two_way_anova_don_microcosm_category <- aov(don ~ system * incubation, data = microcosm_data)
summary(two_way_anova_don_microcosm_category)

# Post-Hoc testing: Both treatment by incubation (due to significant interaction)
one_way_anova_don_microcosm_control_category <- aov(don ~ system, data = microcosm_data_control)
tukey_don_microcosm_control_category <- TukeyHSD(one_way_anova_don_microcosm_control_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_don_microcosm_control_category

one_way_anova_don_microcosm_moderate_category <- aov(don ~ system, data = microcosm_data_moderate)
tukey_don_microcosm_moderate_category <- TukeyHSD(one_way_anova_don_microcosm_moderate_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_don_microcosm_moderate_category

one_way_anova_don_microcosm_high_category <- aov(don ~ system, data = microcosm_data_high)
tukey_don_microcosm_high_category <- TukeyHSD(one_way_anova_don_microcosm_high_category, "system", ordered = FALSE, conf.level = 0.95)
tukey_don_microcosm_high_category
...
### Partial Correlation Analysis: FLNF vs. Diversity  -------------
# Perform partial correlation between diazotroph alpha-diversity~n.fixation when controlling for (doc.don.mbc)
microcosm_data_nona <- microcosm_data %>% drop_na(doc, don, mbc)

pcor_fixation_diversity_microcosm <- pcor.test(microcosm_data_nona$shan, microcosm_data_nona$n.fix, microcosm_data_nona[,c("doc", "don", "mbc", "mbn")], method = "spearman")

pcor_fixation_diversity_microcosm

### Partial Correlation Analysis: MBC normalized FLNF vs. Diversity ---------------------------------------
microcosm_data_nona <- microcosm_data %>% drop_na(doc, don, mbc, n.fix.mbc)

pcor_normfixation_diversity_microcosm <- pcor.test(microcosm_data_nona$shan, microcosm_data_nona$n.fix.mbc, microcosm_data_nona[,c("doc", "don", "mbn")], method = "spearman")
...
merge_effects_category_microcosm <- list(category_diversity_effects_microcosm_annual,
                                         category_diversity_effects_microcosm_perennial,
                                         category_diversity_effects_microcosm_forest)
merge_effects_category_microcosm <- merge_effects_category_microcosm %>% reduce(full_join)
...
merge_tukey_diversity_microcosm <- merge_tukey_diversity_microcosm %>% reduce(full_join)
...
microcosm_data %>%
  ...
  scale_fill_manual(values=c('#F0C96C', '#6E9867', '#5B65A7'), name = "Land-use Category",
                    labels = c("Annual", "Perennial", "Forest")) +
  ylab("Hill Number (q=1)") +
  xlab("Biodiversity Reduction Treatment") +
  scale_x_discrete(labels=c("", "Control", "", "", "Moderate", "", "", "High", "")) +
...
merge_tukey_flnf_microcosm <- merge_tukey_flnf_microcosm %>% reduce(full_join)
...
colnames(letters_df_diversity_microcosm)[1] <- "Letter" #Reassign column name
letters_df_diversity_microcosm$system_incubation <- rownames(letters_df_diversity_microcosm) 
letters_df_diversity_microcosm$placement <- c(14, 14, 14, 14, 14, 14, 14, 14, 14)
...
merge_tukey_mbc_microcosm <- merge_tukey_mbc_microcosm %>% reduce(full_join)
...
merge_tukey_mbn_microcosm <- merge_tukey_mbn_microcosm %>% reduce(full_join)
...
#### EOC #####
# Obtain mean doc values for each system (annual, perennial, forest) in the summer sampling date 
lm_doc_category_microcosm_control <- lm(doc ~ system + 0, data = microcosm_data_control)
summary(lm_doc_category_microcosm_control)

lm_doc_category_microcosm_moderate <- lm(doc ~ system + 0, data = microcosm_data_moderate)
summary(lm_doc_category_microcosm_moderate)

lm_doc_category_microcosm_high <- lm(doc ~ system + 0, data = microcosm_data_high)
summary(lm_doc_category_microcosm_high)

# Add model estimates into dataframe for GGPLOT
est <- c(152.023, 158.195, 175.488)
system <- c("annual", "perennial", "forest")
incubation <- c("control", "control", "control")
category_doc_effects_microcosm_control <- data.frame(est, incubation, system)

est <- c(244.95, 396.05, 482.28)
system <- c("annual", "perennial", "forest")
incubation <- c("moderate", "moderate", "moderate")
category_doc_effects_microcosm_moderate <- data.frame(est, incubation, system)

est <- c(219.42, 334.37, 306.50)
system <- c("annual", "perennial", "forest")
incubation <- c("high", "high", "high")
category_doc_effects_microcosm_high <- data.frame(est, incubation, system)

merge_effects_category_microcosm <- list(category_doc_effects_microcosm_control,
                                         category_doc_effects_microcosm_moderate,
                                         category_doc_effects_microcosm_high)
merge_effects_category_microcosm <- merge_effects_category_microcosm %>% reduce(full_join)

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: control hour
letters_df_doc_microcosm_control <- data.frame(multcompLetters(TukeyHSD(aov(doc ~ treatment, data = microcosm_data_control))$treatment[,4])$Letters)

colnames(letters_df_doc_microcosm_control)[1] <- "Letter" #Reassign column name
letters_df_doc_microcosm_control$treatment <- rownames(letters_df_doc_microcosm_control) 

letters_df_doc_microcosm_control$placement <- c(400, 400, 400, 400, 400, 400, 400, 400)
value_map_system <- c("T1" = "annual",
                      "T3" = "annual",
                      "T4" = "annual",
                      "T5" = "perennial",
...
value_map_incubation <-  c("T1" = "control",
                           ...
                           "SF" = "control",
                           "CF" = "control")
letters_df_doc_microcosm_control$system <- value_map_system[letters_df_doc_microcosm_control$treatment]
letters_df_doc_microcosm_control$system <- factor(letters_df_doc_microcosm_control$system, levels = c("annual", "perennial", "forest"))
letters_df_doc_microcosm_control$incubation <- value_map_incubation[letters_df_doc_microcosm_control$treatment]

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: moderate hour
letters_df_doc_microcosm_moderate <- data.frame(multcompLetters(TukeyHSD(aov(doc ~ treatment, data = microcosm_data_moderate))$treatment[,4])$Letters)

colnames(letters_df_doc_microcosm_moderate)[1] <- "Letter" #Reassign column name
letters_df_doc_microcosm_moderate$treatment <- rownames(letters_df_doc_microcosm_moderate) 

letters_df_doc_microcosm_moderate$placement <- c(650, 650, 650, 650, 650, 650, 650, 650)
value_map_system <- c("T1" = "annual",
                      "T3" = "annual",
                      "T4" = "annual",
                      "T5" = "perennial",
...
value_map_incubation <-  c("T1" = "moderate",
                           ...
                           "SF" = "moderate",
                           "CF" = "moderate")
letters_df_doc_microcosm_moderate$system <- value_map_system[letters_df_doc_microcosm_moderate$treatment]
letters_df_doc_microcosm_moderate$system <- factor(letters_df_doc_microcosm_moderate$system, levels = c("annual", "perennial", "forest"))
letters_df_doc_microcosm_moderate$incubation <- value_map_incubation[letters_df_doc_microcosm_moderate$treatment]

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: high hour
letters_df_doc_microcosm_high <- data.frame(multcompLetters(TukeyHSD(aov(doc ~ treatment, data = microcosm_data_high))$treatment[,4])$Letters)

colnames(letters_df_doc_microcosm_high)[1] <- "Letter" #Reassign column name
letters_df_doc_microcosm_high$treatment <- rownames(letters_df_doc_microcosm_high) 

letters_df_doc_microcosm_high$placement <- c(650, 650, 650, 650, 650, 650, 650, 650)
value_map_system <- c("T1" = "annual",
                      "T3" = "annual",
                      "T4" = "annual",
                      "T5" = "perennial",
...
value_map_incubation <-  c("T1" = "high",
                           ...
                           "SF" = "high",
                           "CF" = "high")
letters_df_doc_microcosm_high$system <- value_map_system[letters_df_doc_microcosm_high$treatment]
letters_df_doc_microcosm_high$system <- factor(letters_df_doc_microcosm_high$system, levels = c("annual", "perennial", "forest"))
letters_df_doc_microcosm_high$incubation <- value_map_incubation[letters_df_doc_microcosm_high$treatment]

# Merge all letters together
merge_tukey_doc_microcosm <- list(letters_df_doc_microcosm_control,
                                  letters_df_doc_microcosm_moderate,
                                  letters_df_doc_microcosm_high)
merge_tukey_doc_microcosm <- merge_tukey_doc_microcosm %>% reduce(full_join)

# Reorder Treatments
merge_effects_category_microcosm$system <- factor(merge_effects_category_microcosm$system, levels = c("annual", "perennial", "forest"))
merge_effects_category_microcosm$incubation <- factor(merge_effects_category_microcosm$incubation, levels = c("control", "moderate", "high"))
merge_tukey_doc_microcosm$incubation <- factor(merge_tukey_doc_microcosm$incubation, levels = c("control", "moderate", "high"))

microcosm_doc_plot <- microcosm_data %>%
  ggplot(aes(x=treatment, y = doc, fill = treatment)) +
  geom_boxplot(alpha = 0.90, outlier.shape = NA) +
  facet_grid(incubation ~system, scales = 'free_x') +
  geom_text(data = merge_tukey_doc_microcosm, 
            aes(x=treatment, y=placement, label=Letter), 
            size =5.5, color="black", fontface="bold") +
  geom_hline(aes(yintercept=est), linetype = 'dashed', 
             col = 'black', linewidth = 1.25,
...
letters_df_diversity_microcosm <- data.frame(multcompLetters(TukeyHSD(aov(doc ~ system_incubation, data = microcosm_data))$system_incubation[,4])$Letters)

colnames(letters_df_diversity_microcosm)[1] <- "Letter" #Reassign column name
letters_df_diversity_microcosm$system_incubation <- rownames(letters_df_diversity_microcosm) 
letters_df_diversity_microcosm$placement <- c(600, 600, 600, 600, 600, 600, 600, 600, 600)
...
microcosm_data %>%
  ggplot(aes(x=system_incubation, y = doc)) +
  geom_boxplot(aes(fill = system), outlier.shape = NA) +
  geom_text(data = letters_df_diversity_microcosm, 
            aes(x = system_incubation, y=placement, label=Letter), 
            size =5.5, color="black", fontface="bold") +
...
#### ETN #####
# Obtain mean don values for each system (annual, perennial, forest) in the summer sampling date 
lm_don_category_microcosm_control <- lm(don ~ system + 0, data = microcosm_data_control)
summary(lm_don_category_microcosm_control)

lm_don_category_microcosm_moderate <- lm(don ~ system + 0, data = microcosm_data_moderate)
summary(lm_don_category_microcosm_moderate)

lm_don_category_microcosm_high <- lm(don ~ system + 0, data = microcosm_data_high)
summary(lm_don_category_microcosm_high)

# Add model estimates into dataframe for GGPLOT
est <- c(15.389, 9.586, 8.019)
system <- c("annual", "perennial", "forest")
incubation <- c("control", "control", "control")
category_don_effects_microcosm_control <- data.frame(est, incubation, system)

est <- c(72.396, 137.570, 66.096)
system <- c("annual", "perennial", "forest")
incubation <- c("moderate", "moderate", "moderate")
category_don_effects_microcosm_moderate <- data.frame(est, incubation, system)

est <- c(63.742, 103.566, 36.575)
system <- c("annual", "perennial", "forest")
incubation <- c("high", "high", "high")
category_don_effects_microcosm_high <- data.frame(est, incubation, system)

merge_effects_category_microcosm <- list(category_don_effects_microcosm_control,
                                         category_don_effects_microcosm_moderate,
                                         category_don_effects_microcosm_high)
merge_effects_category_microcosm <- merge_effects_category_microcosm %>% reduce(full_join)

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: control hour
letters_df_don_microcosm_control <- data.frame(multcompLetters(TukeyHSD(aov(don ~ treatment, data = microcosm_data_control))$treatment[,4])$Letters)

colnames(letters_df_don_microcosm_control)[1] <- "Letter" #Reassign column name
letters_df_don_microcosm_control$treatment <- rownames(letters_df_don_microcosm_control) 

letters_df_don_microcosm_control$placement <- c(200, 200, 200, 200, 200, 200, 200, 200)
value_map_system <- c("T1" = "annual",
                      "T3" = "annual",
                      "T4" = "annual",
                      "T5" = "perennial",
...
value_map_incubation <-  c("T1" = "control",
                           ...
                           "SF" = "control",
                           "CF" = "control")
letters_df_don_microcosm_control$system <- value_map_system[letters_df_don_microcosm_control$treatment]
letters_df_don_microcosm_control$system <- factor(letters_df_don_microcosm_control$system, levels = c("annual", "perennial", "forest"))
letters_df_don_microcosm_control$incubation <- value_map_incubation[letters_df_don_microcosm_control$treatment]

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: moderate hour
letters_df_don_microcosm_moderate <- data.frame(multcompLetters(TukeyHSD(aov(don ~ treatment, data = microcosm_data_moderate))$treatment[,4])$Letters)

colnames(letters_df_don_microcosm_moderate)[1] <- "Letter" #Reassign column name
letters_df_don_microcosm_moderate$treatment <- rownames(letters_df_don_microcosm_moderate) 

letters_df_don_microcosm_moderate$placement <- c(200, 200, 200, 200, 200, 200, 200, 200)
value_map_system <- c("T1" = "annual",
                      "T3" = "annual",
                      "T4" = "annual",
                      "T5" = "perennial",
...
value_map_incubation <-  c("T1" = "moderate",
                           ...
                           "SF" = "moderate",
                           "CF" = "moderate")
letters_df_don_microcosm_moderate$system <- value_map_system[letters_df_don_microcosm_moderate$treatment]
letters_df_don_microcosm_moderate$system <- factor(letters_df_don_microcosm_moderate$system, levels = c("annual", "perennial", "forest"))
letters_df_don_microcosm_moderate$incubation <- value_map_incubation[letters_df_don_microcosm_moderate$treatment]

# Add Tukey's Post-Hoc Comparisons for each MCSE Treatment: high hour
letters_df_don_microcosm_high <- data.frame(multcompLetters(TukeyHSD(aov(don ~ treatment, data = microcosm_data_high))$treatment[,4])$Letters)

colnames(letters_df_don_microcosm_high)[1] <- "Letter" #Reassign column name
letters_df_don_microcosm_high$treatment <- rownames(letters_df_don_microcosm_high) 

letters_df_don_microcosm_high$placement <- c(200, 200, 200, 200, 200, 200, 200, 200)
value_map_system <- c("T1" = "annual",
                      "T3" = "annual",
                      "T4" = "annual",
                      "T5" = "perennial",
...
value_map_incubation <-  c("T1" = "high",
                           ...
                           "SF" = "high",
                           "CF" = "high")
letters_df_don_microcosm_high$system <- value_map_system[letters_df_don_microcosm_high$treatment]
letters_df_don_microcosm_high$system <- factor(letters_df_don_microcosm_high$system, levels = c("annual", "perennial", "forest"))
letters_df_don_microcosm_high$incubation <- value_map_incubation[letters_df_don_microcosm_high$treatment]

# Merge all letters together
merge_tukey_don_microcosm <- list(letters_df_don_microcosm_control,
                                  letters_df_don_microcosm_moderate,
                                  letters_df_don_microcosm_high)
merge_tukey_don_microcosm <- merge_tukey_don_microcosm %>% reduce(full_join)

# Reorder Treatments
merge_effects_category_microcosm$system <- factor(merge_effects_category_microcosm$system, levels = c("annual", "perennial", "forest"))
merge_effects_category_microcosm$incubation <- factor(merge_effects_category_microcosm$incubation, levels = c("control", "moderate", "high"))
merge_tukey_don_microcosm$incubation <- factor(merge_tukey_don_microcosm$incubation, levels = c("control", "moderate", "high"))

microcosm_don_plot <- microcosm_data %>%
  ggplot(aes(x=treatment, y = don, fill = treatment)) +
  geom_boxplot(alpha = 0.90, outlier.shape = NA) +
  facet_grid(incubation ~system, scales = 'free_x') +
  geom_text(data = merge_tukey_don_microcosm, 
            aes(x=treatment, y=placement, label=Letter), 
            size =5.5, color="black", fontface="bold") +
  geom_hline(aes(yintercept=est), linetype = 'dashed', 
             col = 'black', linewidth = 1.25,
  ...
  scale_fill_manual(values=c('#EEAC4E','#D7785D','#9C7C50', '#577753',
                    ...
                    labels = c("T1: Conventional Corn/Soy/Wheat", 
                               ...
                               "SF: Successional Forest", "DF: Deciduous Forest", 
                               "CF: Coniferous Forest")) +
  ylab("Extractable Organic Nitrogen (\u00b5g/g soil)") +
  xlab("") +
  theme_bw() +
  theme(axis.title.x = element_text(size = 14), 
        title = element_text(size = 14),
...
en_coord_cont_sig <- en_coord_cont %>% filter(pval < 0.05)
# Add names of relevant vectors to the dataframe -- for the ggplot
en_coord_cont_sig$name <- c("N.fix", "MBC", "MBN", "ETC", "ETN")
...
microcosm_nmds_ordination <- data.scores %>%
  ...
  scale_fill_manual(values=c('#F0C96C', '#6E9867', '#5B65A7'), name = "Land-use Category",
                    labels = c("Annual", "Perennial", "Forest")) +
  scale_shape_manual(values =c(16, 17, 18), name = "Biodiversity Reduction", 
                     labels = c("Control", "Moderate", "High")) +
  geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
               data = en_coord_cont_sig, size =1.25, alpha = 0.7, colour = "black") +
  geom_label(data = en_coord_cont_sig, aes(x = NMDS1, y = NMDS2), colour = "black", 
             fontface = "bold", size = 5.5, label = en_coord_cont_sig$name, fill = "white") + 
  guides(fill = guide_legend(override.aes = list(shape=c(16)))) +
  ggtitle("Biodiversity Reduction Experiment") +
  theme_bw() +
  theme(axis.title.x = element_text(size = 15)) + 
  theme(axis.title.y = element_text(size = 15)) + 
  theme(axis.text.x = element_text(size = 12, color = "black"))  + 
...
microcosm_flnf_div <- microcosm_data %>%
  ...
  scale_color_manual(values=c('#EEAC4E','#D7785D','#9C7C50', '#577753',
                     ...
                     labels = c(
                       ...
                       "T4: Bio-Based Corn/Soy/Wheat", "T5: Perennial Poplar", "T7: Early Successional Grassland",
                       "SF: Successional Forest", "CF: Coniferous Forest", "DF: Deciduous Forest")) +
  scale_shape_manual(values =c(16, 15, 18), name = "Biodiversity Reduction", 
                     labels = c("Control", "Moderate", "High")) +
  guides(fill = guide_legend(override.aes = list(shape=c(21)))) +
  xlab("Hill Number (q=1)") + 
  ylab("Nitrogen Fixation Rate (\u00b5g/g soil)") +
...
microcosm_category_flnf_div <- microcosm_data %>%
  ...
  facet_grid(~system) +
  geom_text(x=420, y = 15, aes(label = est), size = 5, data = model.df) +
  geom_text(x=420, y = 14, aes(label = R), size = 5, data = model.df) +
  geom_text(x=420, y = 13, aes(label = p), size = 5, data = model.df) +
  geom_smooth(method = 'lm') +
  scale_color_manual(values=c('#EEAC4E','#D7785D','#9C7C50', '#577753',
                             '#1A4C76', '#86A592', '#B0381D','#577B96'), name = "MCSE Treatment",
                    labels = c(
...
R <- c("Adj-R.sq = 0.16", "Adj-R.sq = 0.42", "Adj-R.sq < 0")
p <- c("p = 0.01", "p < 0.001 ", "p = 0.83")
system <- c("annual", "perennial", "forest")
...
ggsave(filename = "mcse_EOC.png", plot = mcse_doc_plot, w = 10, h = 5, dpi = 400)
ggsave(filename = "mcse_ETN.png", plot = mcse_don_plot, w = 10, h = 5, dpi = 400)
...
ggsave(filename = "microcosm_EOC.png", plot = microcosm_doc_plot, w = 10, h = 7, dpi = 400)
ggsave(filename = "microcosm_ETN.png", plot = microcosm_don_plot, w = 10, h = 7, dpi = 400)
